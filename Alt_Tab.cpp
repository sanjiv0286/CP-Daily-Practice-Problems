...
string getAltTabOrder(int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        // Check if the program is already in the stack
        // If it is, remove it from the stack
        stack<string> tempStack;
        while (!programStack.empty() && programStack.top() != program)
        {
            tempStack.push(programStack.top());
            programStack.pop();
        }
        if (!programStack.empty())
            programStack.pop();

        // Push the program back to the stack
        while (!tempStack.empty())
        {
            programStack.push(tempStack.top());
            tempStack.pop();
        }
    ...
    while (!programStack.empty())
    {
        ...
        programStack.pop();
        altTabOrder += program.substr(program.length() - 2, 2);
    }
...
int main()
{
    ...
    string result = getAltTabOrder(N);
    cout << result << endl;
